...
export default function CreateProjectWizard() {
  ...
  const handleSubmitFinal = async () => {
    try {
      // Transform gateways data to match backend format
      const transformedGateways = {};
      Object.entries(projectData.gateways).forEach(([key, value]) => {
        if (value.enabled) {
          transformedGateways[key] = {
            enabled: true,
            config: value.config || {}
          };
        }
      ...
      const payload = {
        ...
        gstInfo: projectData.gstInfo
      };

      console.log('Submitting project:', payload); // Debug log
  ...
  const handleNextFromStep2 = () => {
    // Validate at least one gateway is enabled and configured
    const enabledGateways = Object.values(projectData.gateways).filter(
      gw => gw.enabled && gw.configured
    );

    if (enabledGateways.length === 0) {
      alert("Please enable and configure at least one payment gateway");
      return;
    }
