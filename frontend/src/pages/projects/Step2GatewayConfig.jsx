...
export default function Step2GatewayConfig({ data, update, next, back }) {
  ...
  const gatewayDefinitions = {
    ...
    razorpay: {
      label: "Razorpay",
      baseFields: ["RAZORPAY_KEY_ID", "RAZORPAY_KEY_SECRET"], // ← Fixed field names
    },
  };
  ...
  const handleToggle = (key, enabled) => {
    const cur = localGateways[key] || { enabled: false, config: {}, configured: false };
    const updated = {
      ...localGateways,
      [key]: {
        ...cur,
        ...
        config: cur.config || {}, // ← Changed from 'fields' to 'config'
      },
    };
  ...
  const updateField = (gatewayKey, field, value) => {
    const gw = localGateways[gatewayKey] || { enabled: true, config: {}, configured: false };
    const newConfig = { ...(gw.config || {}), [field]: value }; // ← Changed to 'config'

    const updated = {
      ...localGateways,
      [gatewayKey]: { ...gw, config: newConfig },
    };

    syncToParent(updated);
  };

  const replaceAllFields = (gatewayKey, newConfigObject) => {
    const gw = localGateways[gatewayKey] || { enabled: true, config: {}, configured: false };
    const updated = {
      ...localGateways,
      [gatewayKey]: { ...gw, config: newConfigObject }, // ← Changed to 'config'
    };
    syncToParent(updated);
  };
    ...
    <div className="space-y-10">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {Object.entries(gatewayDefinitions).map(([key, def]) => {
          const val = localGateways[key] || { enabled: false, config: {}, configured: false };
          return (
            <GatewayCard
              key={key}
              gatewayKey={key}
      ...
      {drawerOpen && (
        <GatewayDrawer
          ...
          fields={localGateways[drawerOpen]?.config || {}} // ← Changed to 'config'
